...
DLIST_dict              (void)
{
   ...
   DEBUG_GRAF   yLOG_value   ("page_ends" , rc);
   --rce;  if (rc < 0) {
      DEBUG_GRAF   yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   DEBUG_GRAF   yLOG_info    ("x_beg"     , x_beg);
   DEBUG_GRAF   yLOG_info    ("x_end"     , x_end);
   glPushMatrix    (); {
      x = x_left;
      y =  -75;
      DICT__by_english  (x_beg, &x_word);
      while (x_word != NULL) {
         DEBUG_GRAF   yLOG_complex ("x_word"    , "%4d %-20.20s %s", i, x_word->d_english, x_word->d_gregg);
         DEBUG_GRAF   yLOG_complex ("show at"   , "%4d, %6.2fx, %6.2fy", c, x, y);
         glPushMatrix    (); {
            glTranslatef (x, y + 15, -50);
            glColor4f    (0.5, 0.5, 0.5, 0.3);
            /*> strlcpy  (t, x_word->d_shown, LEN_TITLE);                               <*/
            strlcpy  (t, x_word->d_english, LEN_TITLE);
         ...
         PREFIX_shown (x_word->d_prefix, t);
         strlcpy (x_full, t, LEN_HUND);
         BASE_show    (x_word->d_base  , t);
         strlcat (x_full, t, LEN_HUND);
         SUFFIX_shown (x_word->d_suffix, t);
         strlcat (x_full, t, LEN_HUND);
         DEBUG_GRAF   yLOG_info    ("x_full"    , x_full);
         PAGE_gregg_word (SHAPE_DRAW, x_full, &x, &y);
         ++c;
         ++i;
